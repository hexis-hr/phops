...
function uuid_from_binary ($uuid) {
  ...
  // 8-4-4-4-12
  return substr($value, 0, 8) . '-' . substr($value, 8, 4) . '-' . substr($value, 12, 4)
    . '-' . substr($value, 16, 4) . '-' . substr($value, 20, 12);
}
 ...
 * Equivalents: 0 = o, 1 = i = l, b = p, d = t, g = h = k, m = n, f = v, s = z, r is banned
 */
...
function hamming_encodeNumber ($number) {
  ...
  $result = hamming_encode(str_pad($binaryString, strlen($binaryString) + 4
    - (strlen($binaryString) % 4), '0', STR_PAD_LEFT));
...
function hamming_decodeNumber ($number) {
  ...
  $number = floor($number);
  $binaryString = base_convert($number, 10, 2);
  $result = hamming_decode(str_pad($binaryString, strlen($binaryString) + 7
    - (strlen($binaryString) % 7), '0', STR_PAD_LEFT));
  return base_convert($result, 2, 10);
}
...
function setBit ($string, $offset) {
  while (strlen($string) * 8 - 1 < $offset)
    $string .= chr(0);
  return substr($string, 0, floor($offset / 8)) . chr(ord($string[floor($offset / 8)]) | pow(2, 7 - $offset % 8))
    . substr($string, floor($offset / 8) + 1);
}

function unsetBit ($string, $offset) {
  while (strlen($string) * 8 - 1 < $offset)
    $string .= chr(0);
  return substr($string, 0, floor($offset / 8))
    . chr(ord($string[floor($offset / 8)]) & (255 - pow(2, 7 - $offset % 8)))
    . substr($string, floor($offset / 8) + 1);
}
